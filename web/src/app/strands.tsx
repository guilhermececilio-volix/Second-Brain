...
// Componente Strands do React Bits (reactbits.dev/animations/strands).
// Fios de luz animados via WebGL (OGL). O CSS original (.strands-container) foi
// embutido como estilo inline — são só regras de layout.
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    // dpr:1 — o shader usa uResolution em pixels CSS, mas gl_FragCoord está em
    // pixels do buffer. Em telas retina (dpr 2) isso descasa e o padrão se
    // repete "lado a lado". Fixar dpr em 1 mantém buffer == CSS == uResolution.
    const renderer = new Renderer({ alpha: true, premultipliedAlpha: true, antialias: true, dpr: 1 });
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColors: { value: buildPalette(p.colors as string[]) },
        uColorCount: { value: Math.min((p.colors as string[]).length, MAX_COLORS) },
        uStrandCount: { value: Math.min(p.count as number, MAX_STRANDS) },
        uSpeed: { value: speed },
